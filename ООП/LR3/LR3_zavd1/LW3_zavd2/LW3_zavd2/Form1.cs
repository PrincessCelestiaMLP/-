...

{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            nameBox.Leave += button_Check;
            last_nameBox.Leave += button_Check;
            yearBox.Leave += button_Check;
            datePicker.Leave += button_Check;
            timeBox.Leave += button_Check;
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            ticket.Init(nameBox.Text, last_nameBox.Text, selectedAttraction, int.Parse(yearBox.Text), datePicker.Value.Date, TimeSpan.Parse(timeBox.Text));
        ...
        private void назвіАтракціонуToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            // Створюємо список всіх імен
            List<string> allNames = tickets.Select(t => t.NameAttraction.NameAttraction).ToList();
            ...
            using (var searchForm = new NameSearchForm(allNames))
            {
                if (searchForm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (var ticket in tickets)
                    {
                        if (ticket.NameAttraction.NameAttraction.Equals(name, StringComparison.OrdinalIgnoreCase))
                        {
                            int rowIndex = dataGridView2.Rows.Add();
                            DataGridViewRow row = dataGridView2.Rows[rowIndex];
        ...
        {
            int ind = e.RowIndex;
            AttractionViewForm form = new AttractionViewForm(Attractions,ind);
            form.ShowDialog();
        }
    }
}
